      ...
      {mods.map((m,i)=><Card key={i} style={{marginBottom:14,padding:16}}>
        ...
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            ...
            <tbody>
              {m.vins?.map((v,j)=><tr key={'v'+j}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(v.offset)}</td><td><Tag color={C.gn}>VIN {j+1}</Tag></td><td style={{padding:'5px 10px',color:C.gn,fontWeight:700,fontSize:12}}>{v.vin}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>{v.mirrored?'17B Mirrored':'17B ASCII'}{v.crcOk!==undefined&&<Tag color={v.crcOk?C.gn:C.er} style={{marginLeft:6}}>CRC8 {v.crcOk?'✓':'✗'}</Tag>}</td></tr>)}
              {m.partialVins?.map((pv,j)=><tr key={'pv'+j}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(pv.offset)}</td><td><Tag color={C.a2}>TAIL {j+1}</Tag></td><td style={{padding:'5px 10px',color:C.a2,fontWeight:700,fontSize:12}}>{pv.tail}</td><td style={{padding:'5px 10px',color:pv.crcOk?C.gn:C.er,fontSize:12}}>8B partial CRC {pv.crcOk?'✓':'✗'}</td></tr>)}
              ...
              {m.secretKey&&<tr><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(m.secretKey.offset)}</td><td><Tag color={C.a4}>SECRET</Tag></td><td style={{padding:'5px 10px',color:C.a4,fontWeight:700,fontSize:12}}>{m.secretKey.hex}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>8B sync key {m.keyConsistent?'✓':'✗'}</td></tr>}
              {m.vehicleSecret&&<tr><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(m.vehicleSecret.offset)}</td><td><Tag color={C.a4}>SECRET</Tag></td><td style={{padding:'5px 10px',color:C.a4,fontWeight:700,fontSize:12}}>{m.vehicleSecret.hex}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>{m.vehicleSecret.endian}-endian 16B</td></tr>}
              {m.skey&&!m.vehicleSecret&&!m.secretKey&&<tr><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(m.skoff)}</td><td><Tag color={C.a4}>SECRET</Tag></td><td style={{padding:'5px 10px',color:m.skb?C.tm:C.a4,fontWeight:700,fontSize:12}}>{m.skb?'ERASED':hxb(m.skey)}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>16B key</td></tr>}
              {m.transponderKeys?.map((tk,j)=><tr key={'t'+j}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(tk.offset)}</td><td><Tag color={C.a1}>FOBIK {j+1}</Tag></td><td style={{padding:'5px 10px',color:C.a1,fontSize:12}}>{tk.hex}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>Transponder</td></tr>)}
              {m.immoKeys?.map((ik,j)=><tr key={'k'+j}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(ik.offset)}</td><td><Tag color={C.a1}>IMMO {j+1}</Tag></td><td style={{padding:'5px 10px',color:C.a1,fontSize:12}}>{ik.hex}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>IMMO entry</td></tr>)}
              {m.zzzzTamper&&<tr><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(m.zzzzTamper.offset)}</td><td><Tag color={C.wn}>TAMPER</Tag></td><td style={{padding:'5px 10px',color:m.zzzzTamper.intact?C.gn:C.wn,fontSize:12}}>{m.zzzzTamper.hex} — {m.zzzzTamper.intact?'INTACT':'CLEARED'}</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>ZZZZ</td></tr>}
              ...
              {m.sec16s?.map(s=><tr key={'s16-'+s.slot}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(s.offset)}</td><td><Tag color={s.blank?C.tm:s.slot===1?C.sr:C.a4}>SEC16-{s.slot}</Tag></td><td style={{padding:'5px 10px',fontFamily:"'JetBrains Mono'",fontSize:10,color:s.blank?C.tm:C.sr,fontWeight:700,wordBreak:'break-all'}}>{s.blank?'(blank)':s.hex}</td><td style={{padding:'5px 10px',fontSize:12,color:C.tm}}>{!s.blank&&<span>CS:{s.cs.toString(16).toUpperCase().padStart(4,'0')} {s.csOk!==undefined&&<Tag color={s.csOk?C.gn:C.er}>CS {s.csOk?'OK ✓':'FAIL ✗'}</Tag>}</span>}</td></tr>)}
              ...
              {m.pcmSec6&&<tr><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>0x03C4</td><td><Tag color={m.pcmSec6.markerOk?C.gn:C.er}>PCM-MARK</Tag></td><td style={{padding:'5px 10px',fontFamily:"'JetBrains Mono'",fontSize:11,color:m.pcmSec6.markerOk?C.gn:C.er,fontWeight:700}}>{m.pcmSec6.markerHex}</td><td style={{padding:'5px 10px',color:m.pcmSec6.markerOk?C.gn:C.er,fontSize:12,fontWeight:700}}>{m.pcmSec6.markerOk?'✓ FF FF FF AA':'✗ MISSING'}</td></tr>}
              ...
              {m.pcmSec6&&!m.pcmSec6.markerOk&&!m.pcmSec6.blank&&m.pcmSec6.classification?.populated&&<tr><td colSpan={4} style={{padding:'8px 10px'}}><div style={{padding:'8px 10px',borderRadius:8,background:C.er+'14',border:'1px solid '+C.er+'55',fontSize:11,color:C.tx,lineHeight:1.45}}><span style={{color:C.er,fontWeight:800}}>⚠ Secret bytes present but marker missing</span> — the 6 SEC bytes look populated, but the canonical <span style={{fontFamily:"'JetBrains Mono'",fontWeight:700}}>FF FF FF AA</span> marker @ 0x03C4 is absent, so the PCM bootloader still treats this dump as IMMO_DAMAGED. Apply a BCM→PCM SEC6 sync to restamp the marker.</div></td></tr>}
              ...
              {m.runtimeCounters&&Object.entries(m.runtimeCounters).map(([k,v])=><tr key={k}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fmtOff(v.offset)}</td><td><Tag color={C.tm}>CTR</Tag></td><td style={{padding:'5px 10px',fontSize:12}}>{v.hex} ({v.value.toLocaleString()})</td><td style={{padding:'5px 10px',color:C.tm,fontSize:12}}>{k}</td></tr>)}
    ...
    {advanced&&sub==='security'&&mods.length>0&&(()=>{
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(320px,1fr))',gap:12}}>
        {mods.map((m,i)=>{const vinOk=m.vins?.length>0&&m.vins[0].vin===tv;return<Card key={i} style={{padding:16,borderLeft:'3px solid '+m.color,borderColor:vinOk?C.gn+'50':m.vins?.length&&tv.length===17?C.er+'40':C.bd}}>
          ...
          {m.pcmSec6&&<div style={{fontSize:11,marginBottom:2}}>PCM marker @0x3C4: <span style={{fontFamily:"'JetBrains Mono'",fontSize:10,fontWeight:700,color:m.pcmSec6.markerOk?C.gn:C.er}}>{m.pcmSec6.markerHex}</span> <Tag color={m.pcmSec6.markerOk?C.gn:C.er}>{m.pcmSec6.markerOk?'✓ FF FF FF AA':'✗ MISSING'}</Tag></div>}
          ...
          {m.pcmSec6&&!m.pcmSec6.markerOk&&!m.pcmSec6.blank&&m.pcmSec6.classification?.populated&&<div style={{marginTop:6,padding:'8px 10px',borderRadius:8,background:C.er+'14',border:'1px solid '+C.er+'55',fontSize:11,color:C.tx,lineHeight:1.45}}><span style={{color:C.er,fontWeight:800}}>⚠ Secret bytes present but marker missing</span> — the 6 SEC bytes look populated, but the canonical <span style={{fontFamily:"'JetBrains Mono'",fontWeight:700}}>FF FF FF AA</span> marker @ 0x03C4 is absent, so the PCM bootloader still treats this dump as IMMO_DAMAGED. Apply a BCM→PCM SEC6 sync to restamp the marker.</div>}
          ...
          <Card style={{padding:16,maxHeight:500,overflowY:'auto'}}>
            {diff.groups.slice(0,50).map(([s,e],gi)=>{
              ...
              return<div key={gi} style={{marginBottom:12}}>
                <div style={{fontSize:10,color:C.tm,fontWeight:700}}>{fmtOff(s)}–{fmtOff(e)} ({e-s+1}B)</div>
          ...
          <Card style={{padding:16,borderTop:'3px solid '+C.sr}}>
            ...
            {mods[tt]?.type==='GPEC2A'?<div>
              ...
              return<div>
                ...
                {mods[tt].pcmSec6&&<div style={{fontSize:11,marginBottom:4}}>PCM marker @0x3C4: <span style={{fontFamily:"'JetBrains Mono'",fontSize:10,fontWeight:700,color:mods[tt].pcmSec6.markerOk?C.gn:C.er}}>{mods[tt].pcmSec6.markerHex}</span> <Tag color={mods[tt].pcmSec6.markerOk?C.gn:C.er}>{mods[tt].pcmSec6.markerOk?'✓ FF FF FF AA':'✗ MISSING'}</Tag></div>}
                {mods[tt].pcmSec6&&!mods[tt].pcmSec6.markerOk&&!mods[tt].pcmSec6.blank&&mods[tt].pcmSec6.classification?.populated&&<div style={{marginTop:4,marginBottom:6,padding:'8px 10px',borderRadius:8,background:C.er+'14',border:'1px solid '+C.er+'55',fontSize:11,color:C.tx,lineHeight:1.45}}><span style={{color:C.er,fontWeight:800}}>⚠ Secret bytes present but marker missing</span> — the 6 SEC bytes look populated, but the canonical <span style={{fontFamily:"'JetBrains Mono'",fontWeight:700}}>FF FF FF AA</span> marker @ 0x03C4 is absent, so the PCM bootloader still treats this dump as IMMO_DAMAGED. Apply a BCM→PCM SEC6 sync to restamp the marker.</div>}
