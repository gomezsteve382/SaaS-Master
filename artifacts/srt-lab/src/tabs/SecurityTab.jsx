        ...
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
            ...
            <tbody>
              ...
              {m.sec16s?.map(s=><tr key={'s16-'+s.slot}><td style={{padding:'5px 10px',color:C.a3,fontSize:12}}>{fO(s.offset)}</td><td><Tag color={s.blank?C.tm:s.slot===1?C.sr:C.a4}>SEC16-{s.slot}</Tag></td><td style={{padding:'5px 10px',fontFamily:"'JetBrains Mono'",fontSize:10,color:s.blank?C.tm:C.sr,fontWeight:700,wordBreak:'break-all'}}>{s.blank?'(blank)':s.hex}</td><td style={{padding:'5px 10px',fontSize:12}}>{!s.blank&&s.csOk!==undefined&&<Tag color={s.csOk?C.gn:C.er}>CS {s.csOk?'OK ✓':'FAIL ✗'}</Tag>}</td></tr>)}
