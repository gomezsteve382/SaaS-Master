...
function patchFile(f,nv){
  ...
  for(const s of f.vins){if(s.off+17>out.length){log.push('0x'+s.off.toString(16).toUpperCase()+' SKIPPED — VIN slot needs 17 B, buffer is only '+out.length+' B');continue;}if(s.mirrored){const m=new Uint8Array(17);const _vbp=[...vb];for(let j=0;j<17;j++)m[j]=_vbp[16-j];for(let j=0;j<17;j++)out[s.off+j]=m[j];const _isGen2p=f.type==='RFHUB'&&f.data.length===4096;let _csp;if(_isGen2p){let _wmp=0xAD;const _RFHP=[0x0EA5,0x0EB9,0x0ECD,0x0EE1];for(const _op of _RFHP){if(_op+17>=f.data.length)continue;const _stp=f.data.slice(_op,_op+17);if(_stp.every(b=>b===0xFF||b===0x00))continue;const _scp=f.data[_op+17];if(_scp!==0x00&&_scp!==0xFF){_wmp=rfhGen2DetectMagic(_stp,_scp);if(_wmp===0xDB)_wmp=0xAD;break;}}_csp=rfhGen2VinCs(m,_wmp);}else{_csp=crc8rf(m);}if(s.coff<out.length){out[s.coff]=_csp;log.push('0x'+s.off.toString(16).toUpperCase()+(_isGen2p?' mirrored rfhGen2VinCs':' mirrored CRC8'));}else{log.push('0x'+s.off.toString(16).toUpperCase()+' mirrored — CS SKIPPED (coff 0x'+s.coff.toString(16).toUpperCase()+' past buffer end '+out.length+')');}}else{for(let j=0;j<17;j++)out[s.off+j]=vb[j];if(s.algo==='c16'){if(s.coff+2>out.length){log.push('0x'+s.off.toString(16).toUpperCase()+' CRC16 SKIPPED — coff 0x'+s.coff.toString(16).toUpperCase()+' needs 2 B past buffer end '+out.length);}else{const c=crc16(vb);out[s.coff]=(c>>8)&0xFF;out[s.coff+1]=c&0xFF;log.push('0x'+s.off.toString(16).toUpperCase()+' CRC16');}}else if(s.algo==='c8'){if(s.coff>=out.length){log.push('0x'+s.off.toString(16).toUpperCase()+' CRC8 SKIPPED — coff 0x'+s.coff.toString(16).toUpperCase()+' past buffer end '+out.length);}else{const c=crc8_42(vb);out[s.coff]=c;log.push('0x'+s.off.toString(16).toUpperCase()+' CRC8');}}else log.push('0x'+s.off.toString(16).toUpperCase());}}
...
function writeModuleVIN(data,type,vin,existingVins){
  ...
  else if(type==='BCM'&&existingVins&&existingVins.length>0)offs=existingVins.map(v=>v.off??v.offset);
  else if(type==='BCM')offs=[0x5320,0x5340,0x5360,0x5380];
  else if(isGen1Rfhub)offs=[0x92];
  else if(type==='RFHUB'&&existingVins&&existingVins.length>0)offs=existingVins.map(v=>v.off??v.offset);
  ...
  if(type==='RFHUB'&&hasMirrored){const mr=new Uint8Array(17);const _vba=[...vb];for(let j=0;j<17;j++)mr[j]=_vba[16-j];const _isGen2w=data.length===4096;let _wmw=0xAD;if(_isGen2w){const _RFHO=[0x0EA5,0x0EB9,0x0ECD,0x0EE1];for(const _ow of _RFHO){if(_ow+17>=data.length)continue;const _stw=data.slice(_ow,_ow+17);if(_stw.every(b=>b===0xFF||b===0x00))continue;const _scw=data[_ow+17];if(_scw!==0x00&&_scw!==0xFF){_wmw=rfhGen2DetectMagic(_stw,_scw);if(_wmw===0xDB)_wmw=0xAD;break;}}}offs.forEach(o=>{for(let i=0;i<17;i++)out[o+i]=mr[i];out[o+17]=_isGen2w?rfhGen2VinCs(mr,_wmw):crc8rf(mr);});}
