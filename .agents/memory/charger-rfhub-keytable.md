...
- Field meaning is **unverified**: byte 8 tracks payload like a checksum but no simple sum/xor reproduces it; byte 9 is a small flag usually 0x01 (seen 0x02/0x07/0x08). There is a read-only structure-only parser for it (sibling to the key-table parser, no writer). **Do not relabel fields or add a writer without a bench capture proving semantics** — the "fobs" interpretation is already disproven, so any future relabel needs evidence, not a position-based guess.
