...
25. **VIN + CHECKSUM** — single-file VIN write + checksum patcher (renamed from "VIN PROG" so the checksum capability is discoverable in the tab strip; tab id stays `vinprog`)
26. **WORKFLOW** — vehicle-job orchestrator (Task #501): persistent `vehicleJobs` + `vehicleJobEvents` rows in `@workspace/db`, unified Module Census (ok/missing/mismatch/extra/unknown vs target VIN), Fix Plan builder (BCM → RFHUB → ECM VIN writes, sec16 patch, pairing/verify steps with blockers), pluggable `SecurityAccessSource` (default `LocalAlgoOverJ2534`, plus `FakeSecurityAccessSource` for tests) wired into `tryUnlockWithChain` with NRC 0x36 (lockout) and 0x37 (retry-after-delay) handling, and a Sign-Off summary with totals + ready flag. The tab orchestrates the existing tabs — it does not re-implement BCM/RFHUB/ECM logic.
