...
export default function J2534Scanner() {
        ...
        const handler = (e) => {
          try {
            ...
            if (resp.type === "scanProgress") {
              ...
              return; // Don't resolve for progress messages
            }
  ...
  const connectBridge = useCallback(async () => {
    ...
    try {
      ...
      // List devices
      socket.addEventListener("message", () => {}, { once: true });
      const listHandler = (e) => {
        try {
          const resp = JSON.parse(e.data);
          if (resp.devices) {
    ...
    try {
      ...
      if (resp.success) {
        ...
        (resp.found || []).forEach((m) => {
          log(`✓ ${m.name} TX:0x${m.tx.toString(16).toUpperCase().padStart(3, "0")} RX:0x${m.rx.toString(16).toUpperCase().padStart(3, "0")} VIN:${m.vin || "?"}`, "success");
        });
      } else {
  ...
  const sendUDS = useCallback(
    async (tx, rx, data, label) => {
      try {
        log(`TX [${label}] → 0x${tx.toString(16).toUpperCase()}: ${data.map((b) => b.toString(16).toUpperCase().padStart(2, "0")).join(" ")}`, "tx");
        const resp = await sendCmd({ command: "UDS", txId: tx, rxId: rx, data, timeout: 3000 });
        if (resp.success) {
          log(`RX [${label}] ← 0x${resp.canId.toString(16).toUpperCase()}: ${resp.data.map((b) => b.toString(16).toUpperCase().padStart(2, "0")).join(" ")}`, "rx");
          return resp;
        } else {
  ...
  return (
    <div style={{ background: S.bg, minHeight: "100vh", padding: 20, fontFamily: S.font, color: S.text }}>
        ...
        {status === "disconnected" && (
          <div style={{ background: "#1A1A2E", border: "1px solid #333", borderRadius: 8, padding: 16, marginBottom: 16, fontSize: 12 }}>
            ...
            <div style={{ color: S.dim, lineHeight: 1.8 }}>
              1. Open a terminal on your laptop<br />
              2. Run: <span style={{ color: "#fff" }}>pip install websockets</span><br />
              3. Run: <span style={{ color: "#fff" }}>python j2534_bridge.py</span><br />
              4. Make sure OBDLink EX is plugged in via USB<br />
              5. Click "Connect Bridge" below
            </div>
          </div>
        )}
        ...
        >
          ...
          {logs.length === 0 && (
            <div style={{ color: S.dim, padding: 20, textAlign: "center" }}>
              Start j2534_bridge.py and click Connect Bridge
            </div>
          )}
        </div>
